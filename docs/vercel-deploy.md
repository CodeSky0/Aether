...
4. 构建命令无需修改（`vercel.json` 已配置 `pnpm run vercel-build`：先构建 Editor Host 并复制到 Web 的静态目录，再构建 Next.js，最后将唯一产物复制到根目录 `.next`）。输出目录必须使用根目录 `.next`，不要在 Vercel 项目设置中填写 `apps/@aether/web/.next`。
